  ...
  devtools(
    (set, get) => ({
      ...
      rerandomize: () => {
        ...
        if (
          availableMilestones.length < REQUIRED ||
          availableAwards.length < REQUIRED
        )
          return;
        try {
          set({
        ...
        try {
          const clearedMarkers = Object.fromEntries(
            Object.entries(markers).filter(
              ([slug]) => !draw.milestones.includes(slug),
            ),
          );
          set({
            draw: generateMilestonesOnly(
              availableMilestones,
              draw.awards,
              MAX_PAIR_SYNERGY,
            ),
      ...
      rerandomizeAwards: () => {
        ...
        try {
          const clearedMarkers = Object.fromEntries(
            Object.entries(markers).filter(
              ([slug]) => !draw.awards.includes(slug),
            ),
          );
          set({
            draw: generateAwardsOnly(
              availableAwards,
              draw.milestones,
              MAX_PAIR_SYNERGY,
            ),
      ...
      rerollMilestone: (slug) => {
        ...
        const replacement = selectMilestone(
          availableMilestones,
          remaining,
          draw.awards,
          MAX_PAIR_SYNERGY,
        ...
        if (replacement === null) {
          set({
            error:
              "No valid milestone replacement found. Enable more milestones.",
          });
          return;
        }
        ...
        set({
          draw: { ...draw, milestones: next },
          markers: withoutMarker(markers, slug),
          error: null,
        });
      },
      ...
      rerollAward: (slug) => {
        ...
        if (replacement === null) {
          set({
            error: "No valid award replacement found. Enable more awards.",
          });
          return;
        }
        const next = [...remaining];
      ...
      removeMilestone: (slug) => {
        ...
        if (newAvailM.length < REQUIRED) {
          set({
            availableMilestones: newAvailM,
            draw: { ...draw, milestones: remaining },
            markers: clearedMarkers,
          });
          return;
        }
      ...
      removeAward: (slug) => {
        ...
        if (newAvailA.length < REQUIRED) {
          set({
            availableAwards: newAvailA,
            draw: { ...draw, awards: remaining },
            markers: clearedMarkers,
          });
          return;
        }
      ...
      setAvailable: (newAvailM, newAvailA) => {
        ...
        if (nowEnoughM && nowEnoughA) {
          try {
            newDraw = generateMilestonesAwards(
              newAvailM,
              newAvailA,
              MAX_PAIR_SYNERGY,
            );
    ...
    }),
    {
      ...
      migrate: (persisted, _version) => {
        ...
        const draw = {
          milestones: (persisted.draw?.milestones || []).filter((s) =>
            mSet.has(s),
          ),
        ...
        const markers = Object.fromEntries(
          Object.entries(persisted.markers || {}).filter(([slug]) =>
            validSlugs.has(slug),
          ),
        );
        return {
          ...persisted,
          availableMilestones: (persisted.availableMilestones || []).filter(
            (s) => mSet.has(s),
          ),
          availableAwards: (persisted.availableAwards || []).filter((s) =>
            aSet.has(s),
          ),
          draw,
          markers,
